  ...
  return (
    <div style={{ padding: '20px', backgroundColor: '#f1f5f9', minHeight: '100vh', direction: 'rtl', fontFamily: 'system-ui' }}>
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        ...
        {teams.map((teamName) => {
          // חילוץ סיסמת הלוגין של הצוות
          const teamLoginPass = allMissionsData[teamName]?.loginPass || "לא הוגדרה";
            ...
            <div key={teamName} style={{ marginBottom: '12px', backgroundColor: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }}>
              ...
              >
                <div style={{ textAlign: 'right' }}>
                  ...
                  {/* הצגת סיסמת הלוגין מתחת לשם הצוות */}
                  <div style={{ fontSize: '13px', color: '#dc2626', fontWeight: 'bold', marginTop: '4px' }}>
                    🔑 סיסמת כניסה: {teamLoginPass}
                  </div>
                </div>
              ...
              {activeTeam === teamName && (
                <div style={{ padding: '0 20px 20px', backgroundColor: '#f8fafc' }}>
                  {Object.entries(allMissionsData[teamName])
                    // סינון: מציג רק מפתחות שהם מספרים (תחנות) ולא את ה-loginPass
                    .filter(([id]) => id !== "loginPass") 
                    .map(([id, data]) => (
