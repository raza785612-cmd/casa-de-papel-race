...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    if (userRole === 'mentor' || team.username === 'אביה') navigate(`/mentor/${idFromUrl}`);
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 text-right" dir="rtl">
      {/* Container שמגביל את הרוחב במחשב */}
      <div className="w-full max-w-[380px]"> 
        <div className="mb-10 text-center">
          <div className="text-red-600 text-5xl mb-3 animate-pulse">🎭</div>
          <h1 className="text-white text-4xl font-black italic uppercase tracking-tight">מבצע <span className="text-red-600">המירוץ</span></h1>
          <p className="text-slate-500 text-[10px] mt-2 font-mono tracking-widest uppercase">Authorized_Personnel_Only</p>
        </div>

        <form onSubmit={handleLogin} className="bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl border border-slate-800 space-y-5">
          <div>
          ...
          <button type="submit" disabled={loading} className="w-full py-4 rounded-2xl font-black uppercase transition-all bg-red-600 hover:bg-red-700 text-white active:scale-95 shadow-lg shadow-red-900/20 italic">
